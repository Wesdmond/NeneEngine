    ...
    D3D12_CPU_DESCRIPTOR_HANDLE dsvHandle)
{
    ...
    // G-Buffer textures (4 RT)
    DXGI_FORMAT gbufferFormats[] = { DXGI_FORMAT_R32G32B32A32_FLOAT,  // Position
                                     DXGI_FORMAT_R32G32B32A32_FLOAT,  // Normal
                                     DXGI_FORMAT_R8G8B8A8_UNORM,      // Albedo
                                     DXGI_FORMAT_R32_FLOAT };         // Spec/Roughness

    m_gbufferTextures.resize(4);
    m_rtvHandles.resize(4);
...
void GBuffer::BindForGeometryPass(ID3D12GraphicsCommandList* cmdList)
{
    ...
    cmdList->OMSetRenderTargets(4, m_rtvHandles.data(), true, &m_dsvHandle);
}
